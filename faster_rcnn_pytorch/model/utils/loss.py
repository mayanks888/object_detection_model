...
def delta_loss(pred_delta, target_delta, anchor_label, sigma):
    ...
    weight[pos_index.cpu()] = 1#make weight positive for only positive bounding box values
    ...
    # ignore gt_label==-1 for rpn_loss if anchor box contain -1 gt values
    loss = loss / (anchor_label.data >=0).sum().float()#finding average loss all
    return loss
